import React, { useState } from 'react'
import { Link } from 'react-router-dom'
import './Header.css'
import logo from '../assets/images/hristovi-design-logo.png'

const Header: React.FC = () => {
  const [menuOpen, setMenuOpen] = useState(false)
  const [projectsOpen, setProjectsOpen] = useState(false)

  return (
    <header className="header">
      <div className="header-top">
        <div className="container">
          <div className="header-top-content">
            <span>За запитвания: </span>
            <a href="tel:+359897495025" className="header-contact-link">+359 897 495 025</a>
            <span className="contact-separator">|</span>
            <a href="tel:+359895436948" className="header-contact-link">+359 895 436 948</a>
            <span className="contact-separator">|</span>
            <a href="mailto:office@hristovidesign.bg" className="header-contact-link">office@hristovidesign.bg</a>
          </div>
        </div>
      </div>
      
      <div className="header-main">
        <div className="container">
          <div className="header-content">
            <div className="logo">
              <Link to="/">
                <img src={logo} alt="Hristovi Design" className="logo-img" />
              </Link>
            </div>
            
            <nav className={`nav ${menuOpen ? 'open' : ''}`}>
              <Link to="/" className="nav-link" onClick={() => setMenuOpen(false)}>НАЧАЛО</Link>
              <Link to="/about" className="nav-link" onClick={() => setMenuOpen(false)}>ЗА НАС</Link>
              <Link to="/services" className="nav-link" onClick={() => setMenuOpen(false)}>УСЛУГИ</Link>
              <div 
                className="nav-dropdown"
                onMouseEnter={() => setProjectsOpen(true)}
                onMouseLeave={() => setProjectsOpen(false)}
              >
                <Link to="/projects" className="nav-link" onClick={() => setMenuOpen(false)}>
                  ПРОЕКТИ <span className="dropdown-arrow">▼</span>
                </Link>
                <div className={`dropdown-menu ${projectsOpen ? 'show' : ''}`}>
                  <Link to="/projects/kitchens" className="dropdown-item" onClick={() => setMenuOpen(false)}>Кухни</Link>
                  <Link to="/projects/living" className="dropdown-item" onClick={() => setMenuOpen(false)}>Дневни</Link>
                  <Link to="/projects/bedrooms" className="dropdown-item" onClick={() => setMenuOpen(false)}>Спални</Link>
                  <Link to="/projects/kids" className="dropdown-item" onClick={() => setMenuOpen(false)}>Детски</Link>
                  <Link to="/projects/office" className="dropdown-item" onClick={() => setMenuOpen(false)}>Офиси</Link>
                  <Link to="/projects/moisture" className="dropdown-item" onClick={() => setMenuOpen(false)}>Влагоустойчиви мебели</Link>
                  <Link to="/projects/luxury" className="dropdown-item" onClick={() => setMenuOpen(false)}>Луксозни проекти</Link>
                  <Link to="/projects/interior" className="dropdown-item" onClick={() => setMenuOpen(false)}>Цялостни интериорни решение</Link>
                </div>
              </div>
              <Link to="/contacts" className="nav-link" onClick={() => setMenuOpen(false)}>КОНТАКТИ</Link>
            </nav>
            
            <div className="header-actions">
              <div className="social-links-header">
                <a href="https://www.facebook.com/profile.php?id=61557171145293" target="_blank" rel="noopener noreferrer" className="social-link" title="Facebook">
                  <svg className="social-icon" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </a>
                <a href="https://www.instagram.com/hristovidesign2" target="_blank" rel="noopener noreferrer" className="social-link" title="Instagram">
                  <svg className="social-icon" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path fill="#E4405F" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                  </svg>
                </a>
              </div>
            </div>
            
            <button 
              className="menu-toggle"
              onClick={() => setMenuOpen(!menuOpen)}
            >
              ☰
            </button>
          </div>
        </div>
      </div>
    </header>
  )
}

export default Header
